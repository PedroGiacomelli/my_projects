...
void forward_propagation(char input_weights[N_INPUTS][LEN_HIDDEN_LAYERS], char hidden_weights[LEN_HIDDEN_LAYERS][LEN_HIDDEN_LAYERS], char output_weights[LEN_HIDDEN_LAYERS],
                        char input_bias[LEN_HIDDEN_LAYERS], char hidden_bias[LEN_HIDDEN_LAYERS], char output_bias[N_OUTPUTS],
...
int main() {
    ...
    //unsigned int inputs[N_INPUTS]={251, 2024, 2007, 2161, 2172, 2214, 2207, 2163, 2113, 2112, 2241};
    unsigned int inputs[N_INPUTS]={4097, 4097, 4097, 4097, 4097, 4097, 4097, 4097,40973, 4097, 4097};

    char input_weights[N_INPUTS][LEN_HIDDEN_LAYERS]={
    ...
    char hidden_weights[LEN_HIDDEN_LAYERS][LEN_HIDDEN_LAYERS]={
    ...
    char output_weights[LEN_HIDDEN_LAYERS]={-43, -62, 24, -50, 38, 56, 97, -47, 113, 127, 13, -37, 120, 76, 12};

    char input_bias[LEN_HIDDEN_LAYERS]={-9, 79, -37, 95, 102, -3, -37, 97, 102, -11, -93, -11, 43, 26, 35};
    char hidden_bias[LEN_HIDDEN_LAYERS]={79, 67, -65, -33, -2, -20, -76, 66, -40, -35, 53, -30, 60, -73, 52};
    char output_bias[N_OUTPUTS]={47};
...
for (i=0; i<LEN_HIDDEN_LAYERS; i++){
    ...
    for (j=0; j<N_INPUTS; j++) {
        ...
        activation+=input_weights[j][i]*inputs[j];
...
for (i=0; i<LEN_HIDDEN_LAYERS; i++){
    ...
    for (j=0; j<LEN_HIDDEN_LAYERS; j++){
        ...
        activation+= hidden_weights[i][j]*temp_vals[j];
...
for(i=0; i < LEN_HIDDEN_LAYERS; i++){
    ...
    activation=output_bias[0];

    for (j=0; j < LEN_HIDDEN_LAYERS; j++){

        activation+=output_weights[i]*final_vals[j];
